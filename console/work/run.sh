...
tail -f $install_dir/logs/nsc.log | grep -q 'Security Console web interface ready'
...
if [[ -n $ACTIVATION_LICENSE_FILE ]]; then
    if [[ -z $LICENSE_STATUS ]] || [ $LICENSE_STATUS == "Unlicensed" ]; then
      ...
      echo '##############################'
      RESPONSE=`curl -s --header "Authorization: Basic $USERAUTH" -H "Content-Type: multipart/form-data" -k -XPOST "https://localhost:$CONSOLE_PORT/api/3/administration/license" -F "license="@${ACTIVATION_LICENSE_FILE}`
      LICENSE_STATUS=`curl -s --header "Authorization: Basic $USERAUTH" -XGET -k "https://localhost:$CONSOLE_PORT/api/3/administration/license" | grep -m1 -oP '"status"\s*:\s*"\K[^"]+'`
